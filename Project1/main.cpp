...
void move(std::string str)
{
    ...
    if (str=="e1g1") if (position.find("e1")==-1) move("h1f1"); // -1 when no found in the position string 
...
void loadPosition()
{
    ...
    for(int i=0;i<position.length();i+=5)
      move(position.substr(i,4)); // maybe the computer value for the new position is 
	  								//		appended to the start instead of the end of the first 4 string are sent to be moves
}
